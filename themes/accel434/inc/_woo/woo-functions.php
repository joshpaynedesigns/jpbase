...
<?php }
// add_action( 'woocommerce_single_product_summary', 'mktg434_product_features', 25 );
function mktg434_product_features() {
    global $product;
    ...
    if ( have_rows( 'product_features', $product->ID ) ) {
        ...
        while ( have_rows( 'product_features', $product->ID ) ): the_row();
...
function cgd_shipping_notice() {
    ...
    if ( have_rows( 'sales_notices', 'option' ) ) {
        while( have_rows( 'sales_notices', 'option' ) ): the_row();
            ...
            } else if ( $display_options == 'global' ) {
                echo '<span class="sales-notice">' . $notice_text . '</span>';
            }
        endwhile;
    }
...
add_action( 'mktg434_after_banner', 'cgd_shop_toolbar' );
function cgd_shop_toolbar() {
    ...
    if ( ! empty( $intro_text ) ) {?>
        <div class="mktg434-archive-intro mobile">
            <div class="wrap">
                <?php echo( $intro_text ); ?>
            </div>
        </div>
