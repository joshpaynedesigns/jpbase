...
class Obj_Gmaps_Public {
	...
	public function map_shortcode_markup() {
		...
		foreach ( $posts as $key => $post ) {
			$lat = get_post_meta( $post->ID, '_obj_location_lat', true );
			$lng = get_post_meta( $post->ID, '_obj_location_lng', true );
			...
			if ( $lat && $lng ) {
				...
				$posts[ $key ]->address_components = get_post_meta( $post->ID, '_obj_location_address_components', true );
			}
		}
		...
		} elseif ( $search_by == 'establishment' ) {
			$placeholder = 'Search by Establishment...';
		} elseif ( $search_by == '(cities)' ) {
			$placeholder = 'Search by City...';
		} elseif ( $search_by == '(regions)' ) {
