...
class Obj_Gmaps_Admin {
	...
	function save_location_lat_long( $post_id, $post, $update ) {
		...
		$api_key = get_option( 'obj_api_key_geocode' );
		if ( empty( $api_key ) ) {
			$api_key = get_option( 'obj_api_key' );
		}
		...
		if ( isset( $_POST['obj-google-address'] ) ) {
			...
			$url     = 'https://maps.googleapis.com/maps/api/geocode/json?address=' . $string . '&sensor=false&key=' . $api_key;
			...
			if ( $output->status == 'OK' ) {
				...
				update_post_meta( $post_id, '_obj_location_address_components', $address_components );
				update_post_meta( $post_id, '_obj_location_lat', $latitude );
				update_post_meta( $post_id, '_obj_location_lng', $longitude );
			}
		}
	}
}
